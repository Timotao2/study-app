...
try:
    with sync_playwright() as p:
        ...
        print("invite-code sign-up from the login page")
        page.request.post("http://localhost:5055/admin/code", data=json.dumps({"action":"set","code":"X7C4-DVWM","expires":"2027-01-01"}), headers={"Content-Type":"application/json"})
        ctx2 = b.new_context(); p2 = ctx2.new_page()
        p2.goto("http://localhost:5055/login"); p2.wait_for_selector("#signup", state="visible", timeout=5000)
        check(True, "sign-up section appears when a code is active")
        p2.fill("#ic", "x7c4-dvwm"); p2.fill("#in", "Eva"); p2.click("text=Create my login")
        p2.wait_for_url("**/enroll/**", timeout=5000); check(True, "correct code lands on the enrolment page")
        con = sqlite3.connect(SCRATCH); s = con.execute("SELECT totp_secret FROM accounts WHERE username='Eva'").fetchone()[0]; con.close()
        p2.fill("#c", pyotp.TOTP(s).now()); p2.click("text=Confirm"); p2.wait_for_selector("#step2", state="visible", timeout=5000)
        p2.click("text=Skip for now"); p2.wait_for_selector("#whoBtn:has-text('Eva')", timeout=5000)
        check(not p2.is_visible("#admBtn"), "Eva is in, no admin link")
        ctx2.close()
